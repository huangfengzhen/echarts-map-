...
$(function () {
	...
	// 初始化插件参数及插入插件
	WebVideoCtrl.I_InitPlugin(1700, 900, {
		iWndowType: 2,
		cbSelWnd: function (xmlDoc) {
			g_iWndIndex = $(xmlDoc).find("SelectWnd").eq(0).text();
			var szInfo = "当前选择的窗口编号：" + g_iWndIndex;
	...
	WebVideoCtrl.I_InsertOBJECTPlugin("divPlugin"); //嵌入插件
...
function clickLogin() {
	...
	//需要传4个值   账号  地址 密码  端口  端口默认是80  可以不传
	var iRet = WebVideoCtrl.I_Login(szIP, 1, szPort, szUsername, szPassword, {
		success: function (xmlDoc) {
			showOPInfo(szIP + " 登录成功！");
...
// 	alert(channels.length + "_____通道长度")
// 		(function (index) {
// 			setTimeout(function () {
...
/* function clickStartRealPlay() {
	var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex), //获取开始预览时所点击的index 4个
		szIP = $("#ip").val(),
		iStreamType = parseInt($("#streamtype").val(), 10),
		iChannelID = parseInt($("#channels").val(), 10), // 通道列表ID
		bZeroChannel = $("#channels option").eq($("#channels").get(0).selectedIndex).attr("bZero") == "true" ? true : false, //是否播放0通道
		szInfo = "";
...
$(function () {
	...
	WebVideoCtrl.I_InitPlugin(600, 360, {
	...
	setTimeout(clickStartRealPlay2, 500);

	setTimeout(clickStartRealPlay1, 500);
...
$(document).ready(function () {
	document.getElementById('clickLogin').onclick();
})